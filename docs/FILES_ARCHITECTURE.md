...
When a provider is genuinely unconfigured, file APIs return `FILE_STORAGE_UNAVAILABLE`. When a configured S3-compatible provider rejects or cannot complete an operation, they return `FILE_STORAGE_OPERATION_FAILED` with a generic retry message. Server logs retain only safe operation diagnostics: operation, HTTP status, provider error code, exception type, and provider request ID when available; credentials, signatures, headers, response bodies, and file content are never logged.
