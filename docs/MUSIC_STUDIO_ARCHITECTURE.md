...
| `IMusicGenerationProvider` | Defines the provider adapter contract. Providers receive normalized user intent and return validated audio bytes plus internal usage metadata. The registered Mubert and Stable Audio adapters are disabled until server-only credentials are configured. |
...
The registered adapters target two independent provider contracts behind the same Taslim interface. Mubert submits a bounded request, polls its track resource, and downloads its temporary URL. Stable Audio submits `multipart/form-data` to `POST /v2beta/audio/stable-audio/text-to-audio`, receives a generation ID with HTTP 202, polls `GET /v2beta/audio/results/{id}`, and consumes the binary result immediately. The durable Taslim worker remains the outer asynchronous execution and provides lease renewal, stale-worker protection, cooperative cancellation, and idempotent job creation; the Stable Audio adapter adds bounded transient retries and bounded provider polling. No callback route is added because the current provider contract is already safely represented by polling.

Each adapter checks `MusicGeneration:Enabled`, its provider key, and server-only credentials before making any request. Stable Audio additionally validates the configured HTTPS base URL, model (`stable-audio-3`), and output format (`mp3` or `wav`). It applies the linked job cancellation token, a bounded overall timeout, bounded transient retries, a bounded polling count, and a bounded streamed output download. Neither adapter fabricates a remote cancellation request: cancellation stops Taslim polling cooperatively and the shared worker marks the job and usage transaction cancelled.

The default development and production configuration registers both adapters but leaves `MusicGeneration:Enabled` false and credentials empty. When enabled, Stable Audio returns only bounded audio bytes, a supported MIME/format pair, requested duration metadata, latency, and safe administrator-only metadata. Its documented 26-credit successful-generation price is recorded as internal provider cost using `StableAudioCreditsPerGeneration` and `StableAudioUsdPerCredit`; customer charging remains zero. No raw provider response, API key, or storage key is persisted or logged. The server-only configuration keys are `MusicGeneration:StableAudioApiBaseUrl`, `StableAudioApiKey`, `StableAudioModel`, `StableAudioOutputFormat`, `StableAudioPollIntervalMilliseconds`, `StableAudioMaxPollAttempts`, `StableAudioMaxRetryAttempts`, `StableAudioRetryBaseDelayMilliseconds`, `StableAudioCreditsPerGeneration`, and `StableAudioUsdPerCredit`.

The foundation supports `audio/mpeg`, `audio/wav`, `audio/ogg`, `audio/mp4`, `audio/aac`, and `audio/flac`. A provider that returns another format must be rejected until the format is intentionally added to the server allowlist and the browser playback path is tested. Both adapters request and accept only `mp3` or `wav` and currently default to `mp3`. Before publication, the shared inspector enforces non-empty bounded bytes, MIME/format agreement, and a matching MP3/WAV/container signature; duration remains the requested duration because the existing provider contract has no safe decoder dependency.
...
## Stable Audio error normalization

Stable Audio authentication and quota failures normalize to unavailable behavior; rate limits use an internal rate-limited category; HTTP 400/422 requests normalize to invalid request or rejected prompt when the bounded provider status indicates moderation or safety rejection; timeouts, transport failures, and exhausted polling normalize to unavailable or timeout; malformed audio normalizes to invalid output; and caller cancellation follows the shared `MUSIC_CANCELLED` path. Upstream response bodies are never copied into job messages, usage metadata, or logs.
...
Provider-specific lyric moderation, waveform generation, loudness normalization, transcoding, stems, multi-track projects, editing, waveform visualization, playlisting, webhook callback coordination, and customer billing remain outside this branch. The Mubert and Stable Audio adapters are production-ready at the server boundary but remain disabled until an approved provider account, credentials, private storage configuration, and commercial gate are supplied. Each future feature should preserve the single logical Asset boundary and add a focused representation or related entity only when the product contract requires it.
