...
import { api, type GenerationJob, type MovieProject, type MovieProviderReadiness, type MovieScene, type Project } from "@/lib/api";
  ...
  return (
    <div className="movie-studio-page">
      ...
      {!saved ? (
        ...
        <MovieProductionWorkspace project={saved} job={job} status={status} completed={completed} outputAssetId={outputAssetId} provider={provider} t={t} error={error} working={working} newScene={newScene} setNewScene={setNewScene} addScene={addScene} generateScene={generateScene} reset={reset} />
      )}
    </div>
  );
}

type ProductionProps = { project: MovieProject; job: GenerationJob | null; status: MovieStatus; completed: boolean; outputAssetId: string | null; provider: MovieProviderReadiness | null; t: (key: string, variables?: Record<string, string>) => string; error: string; working: boolean; newScene: { title: string; summary: string }; setNewScene: (value: { title: string; summary: string }) => void; addScene: () => Promise<void>; generateScene: (sceneId: string) => Promise<void>; reset: () => void };

function MovieProductionWorkspace({ project, job, status, completed, outputAssetId, provider, t, error, working, newScene, setNewScene, addScene, generateScene, reset }: ProductionProps) {
    ...
    <div className="movie-production-workspace">
      ...
      <div className="movie-production-grid">
        ...
        <main className="movie-storyboard-panel">
          ...
          {project.scenes.length ? <div className="movie-scene-list">{project.scenes.map((scene, index) => <MovieSceneCard key={scene.id} scene={scene} index={index} t={t} working={working} onGenerate={generateScene} />)}</div> : <div className="movie-storyboard-empty"><div className="movie-empty-thumb"><Film size={23} /></div><h3>{t("movie.storyboardEmpty")}</h3><p>{t("movie.storyboardEmptyText")}</p></div>}
...
function MovieSceneCard({ scene, index, t, working, onGenerate }: { scene: MovieScene; index: number; t: (key: string, variables?: Record<string, string>) => string; working: boolean; onGenerate: (sceneId: string) => Promise<void> }) {
...
function ContinuitySummary({ project, t }: { project: MovieProject; t: (key: string, variables?: Record<string, string>) => string }) {
  ...
  return <section className="movie-continuity-card"><div className="movie-continuity-heading"><div><span className="section-eyebrow">{t("movie.guideEyebrow")}</span><h2>{t("movie.continuity")}</h2></div><Layers3 size={17} /></div><div className="movie-continuity-list">{fields.map(([label, value]) => <div key={label}><span>{label}</span><p>{value || t("movie.notSet")}</p></div>)}</div>{guide.continuityRules && <div className="movie-continuity-rule"><span>{t("movie.continuityRules")}</span><p>{guide.continuityRules}</p></div>}</section>;
}
