...
import { Check, CreditCard, Languages, LogOut, Mail, UserRound } from "lucide-react";
...
export function AccountView() {
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    try {
      await updateProfile({ displayName, preferredLanguage });
      setSaved(true);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : t("account.saveError"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return <div className="account-page">
    <div className="account-header"><div><p className="section-eyebrow">{t("account.eyebrow")}</p><h1>{t("account.title")}</h1><p>{t("account.subtitle")}</p></div><span className="account-avatar">{user?.displayName.slice(0, 1).toUpperCase()}</span></div>
    <div className="account-grid">
      <form className="account-card profile-form" onSubmit={submit}>
        ...
        <label><span>{t("account.preferredLanguage")}</span><div className="select-shell"><Languages size={16} /><select value={preferredLanguage} onChange={(event) => setPreferredLanguage(event.target.value as Locale)}>{locales.map((item) => <option key={item} value={item}>{localeNames[item]}</option>)}</select></div></label>
        {error && <div className="form-error">{error}</div>}
        {saved && <div className="form-success"><Check size={15} /> {t("account.saved")}</div>}
        <button className="primary-button" disabled={saving}>{saving ? t("common.saving") : <><Check size={15} /> {t("common.saveChanges")}</>}</button>
      </form>
      <div className="account-side">
        ...
        <div className="account-card usage-link-card"><div><h2>{t("account.jobsTitle")}</h2><p>{t("account.jobsSubtitle")}</p></div><Link className="secondary-button" href="/account/generation-jobs">{t("account.viewJobs")}</Link></div>
