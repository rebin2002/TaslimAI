...
const labels: AssetCardLabels = { project: "Project", workspace: "Workspace asset", rename: "Edit", archive: "Archive", restore: "Restore", download: "Download", type: "File" };
const asset: Asset = {
...
describe("AssetCard", () => {
  it("renders user-facing metadata and authenticated asset actions without provider internals", () => {
    const html = renderToStaticMarkup(<AssetCard asset={asset} labels={labels} locale="en" onEdit={vi.fn()} onArchive={vi.fn()} onRestore={vi.fn()} />);
  ...
  it("shows restore instead of archive for archived assets", () => {
    const html = renderToStaticMarkup(<AssetCard asset={{ ...asset, status: "Archived", archivedAt: "2026-09-22T01:00:00Z" }} labels={labels} locale="en" onEdit={vi.fn()} onArchive={vi.fn()} onRestore={vi.fn()} />);
