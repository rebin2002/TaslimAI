...
import { canCancelSocialJob, displaySocialProgress, formatSocialPostForCopy, isSocialAssetSelectable, isSocialSourceReady, nextSocialPollDelay, parseSocialJobResult, shouldPollSocialJob, socialStudioState } from "@/lib/socialStudioState";

const extensions = [".pdf", ".docx", ".txt", ".md", ".csv", ".xlsx"];
type Language = "auto" | "en" | "ar" | "ku";
type Platform = "instagram" | "facebook" | "linkedin" | "x" | "tiktok" | "multi";
...
export function SocialStudioView() {
  ...
  return <div className="social-studio-page">
    <div className="social-studio-header"><div><p className="section-eyebrow">{t("social.eyebrow")}</p><h1>{t("social.title")}</h1><p>{t("social.subtitle")}</p></div><span className="social-studio-header-icon"><Share2 size={26} /></span></div>
...
function SocialResult({ result, copyState, onCopy, onCreateAnother, t }: { result: SocialJobResult; copyState: string; onCopy: (post: SocialPost) => void; onCreateAnother: () => void; t: (key: string, values?: Record<string, string>) => string }) {
  return <section className="account-card social-generation-state" aria-live="polite"><div className="social-result-heading"><div><p className="section-eyebrow">{t("social.resultEyebrow")}</p><h2>{result.title || t("social.resultTitle")}</h2><p className="social-result-meta">{t("social.postCount", { count: String(result.postCount ?? result.posts?.length ?? 0) })} · {result.platform || "multi"}</p></div><span className="form-success"><CheckCircle2 size={16} /> {t("social.savedToAssets")}</span></div><div className="social-post-list">{result.posts?.map((post) => <article className="social-post-card" key={post.order}><div className="social-post-card-heading"><span>{post.order}</span><button className="secondary-button" type="button" onClick={() => onCopy(post)}><Copy size={14} /> {copyState === String(post.order) ? <><Check size={14} /> {t("social.copied")}</> : t("social.copy")}</button></div><h3>{post.hook}</h3><p>{post.body}</p>{post.callToAction && <strong className="social-post-cta">{post.callToAction}</strong>}{post.hashtags?.length ? <p className="social-post-tags">{post.hashtags.join(" ")}</p> : null}{post.altText && <details className="social-post-detail"><summary>{t("social.altText")}</summary><p>{post.altText}</p></details>}{post.visualDirection && <details className="social-post-detail"><summary>{t("social.visualDirection")}</summary><p>{post.visualDirection}</p></details>}{post.assetRefs?.length ? <small className="social-post-assets">{t("social.usesAssets")}: {post.assetRefs.join(", ")}</small> : null}</article>)}</div><div className="social-result-actions"><Link className="secondary-button" href="/assets">{t("social.openAssets")}</Link><button className="primary-button" onClick={onCreateAnother}><RefreshCw size={15} /> {t("social.createAnother")}</button></div></section>;
}
