...
const quickActions = [
  { href: "/create/image", labelKey: "navigation.imageStudio", icon: Image },
...
function HomeDashboard() {
  ...
  // Load independently-owned, workspace-scoped dashboard records after session state is available.
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(() => { void loadDashboard(); }, [loadDashboard]);

  const recentItems = useMemo(() => dashboard ? buildHomeRecentItems(dashboard, 8) : [], [dashboard]);
  ...
  return (
    <div className="home-dashboard">
      <section className="home-welcome" aria-labelledby="home-title">
        ...
        <Link href="/chat" className="home-chat-entry">
          <span className="home-chat-entry-icon"><MessageSquare size={19} /></span>
          <span><strong>{t("home.chatCta")}</strong><small>{t("home.chatEntryHint")}</small></span>
          <ArrowUpRight size={17} />
        </Link>
      </section>

      <section className="home-creation-console" aria-label={t("home.createPanelLabel")}>
        <div className="home-console-heading">
          <span className="home-console-icon"><Sparkles size={18} /></span>
          <div><h2>{t("home.title")}</h2><p>{t("home.consoleHint")}</p></div>
        </div>
        <div className="home-idea-input-wrap">
          <input
            className="home-idea-input"
            value={idea}
            onChange={(event) => setIdea(event.target.value)}
            onKeyDown={(event) => { if (event.key === "Enter") void askTaslim(); }}
        ...
        <div className="home-console-footer">
          <span>{t("home.consolePrivacy")}</span>
          <Link href="/chat">{t("home.openChatDirect")} <ArrowUpRight size={14} /></Link>
        </div>
      </section>
      ...
      <section className="home-quick-actions" aria-labelledby="quick-actions-title">
        <div className="home-section-heading">
          <div><p className="section-eyebrow">{t("home.startNowEyebrow")}</p><h2 id="quick-actions-title">{t("home.quickActions")}</h2></div>
          <Link href="/create" className="home-view-all">{t("home.browseStudios")} <ArrowUpRight size={14} /></Link>
        </div>
        <div className="home-quick-grid">
          {quickActions.map((action) => {
            const Icon = action.icon;
            return <Link key={action.href} href={action.href} className="home-quick-action"><span><Icon size={18} /></span><strong>{t(action.labelKey)}</strong><ArrowUpRight size={14} /></Link>;
          })}
        </div>
      </section>
      ...
      <section className="home-continue" aria-labelledby="continue-work-title">
        <div className="home-section-heading home-continue-heading">
          <div>
            <p className="section-eyebrow">{hasWorkingGeneration ? t("home.workInProgress") : t("home.recent")}</p>
            <h2 id="continue-work-title">{t("home.continueTitle")}</h2>
            <p>{t("home.continueDescription")}</p>
          </div>
          <div className="home-continue-links">
            <Link href="/projects">{t("navigation.projects")}</Link>
            <Link href="/chat">{t("navigation.chat")}</Link>
            <Link href="/notifications">{t("navigation.activity")}</Link>
            <Link href="/assets">{t("navigation.assets")}</Link>
          </div>
        </div>
...
function RecentWorkCard({ item, locale, t }: { item: HomeRecentItem; locale: "en" | "ar" | "ku"; t: (key: string, variables?: Record<string, string>) => string }) {
  ...
  return <Link href={item.href} className="home-recent-card">
    ...
    <span className="home-recent-content">
      <span className="home-recent-meta"><span>{kindLabel}</span>{statusLabel && <span className={`home-work-status ${statusClass}`}>{statusLabel}</span>}</span>
      <strong>{item.title}</strong>
      <small>{formatDate(item.timestamp, locale)}</small>
      {item.status && (item.status === "Queued" || item.status === "Running") && <span className="home-progress"><span style={{ width: `${Math.max(0, Math.min(100, item.progressPercent ?? 0))}%` }} /></span>}
    </span>
