...
export type MovieV2TakeDto = { id: string; movieShotId: string; versionNumber: number; label: string; status: string; qualityLevel: string; autoDirectorEnabled: boolean; movieClipId: string | null; generationJobId: string | null; movieProductionVersionId: string | null; assetId: string | null; notes: string | null; selectedAt: string | null; finalizedAt: string | null; createdAt: string; updatedAt: string; approvals: { id: string; userId: string; decision: string; comment: string | null; createdAt: string }[] };
...
export type MovieRegenerationRequest = { id: string; movieShotId: string; targetType: string; targetId: string; actionType: string; requestedStage: MovieProductionStage; reason: string; sourceVersionId: string | null; changedInputsJson: string; compositionJson: string; status: string; createdByUserId: string; confirmedByUserId: string | null; generationJobId: string | null; resultingProductionVersionId: string | null; resultingTakeId: string | null; createdAt: string; confirmedAt: string | null; costPreview: { estimatedProviderCostUsd: number | null; estimatedProviderCostKnown: boolean; currency: string; costEstimateJson: string | null; confirmationRequired: boolean } };
export type MovieShotProduction = { movieShotId: string; currentStage: MovieProductionStage; versions: MovieProductionVersion[]; transitions: MovieProductionStageTransition[]; regenerationRequests: MovieRegenerationRequest[] };
export type MovieSelectiveRegenerationResponse = { request: MovieRegenerationRequest; job: GenerationJob | null; productionVersion: MovieProductionVersion | null; take: MovieV2TakeDto | null };
...
export const api = {
  ...
  createMovieRegenerationRequest: (shotId: string, input: { actionType: string; requestedStage: MovieProductionStage; reason: string; sourceVersionId?: string | null; changedInputsJson: string; compositionJson?: string; estimatedProviderCostUsd?: number | null; internalCostEstimate?: unknown }, idempotencyKey = requestId()) => request<MovieSelectiveRegenerationResponse>(`/api/movie-studio/shots/${shotId}/regeneration-requests`, generationInit({ method: "POST", body: JSON.stringify(input) }, idempotencyKey), true),
