...
export type MovieShot = { id: string; sequence: number; description: string; cameraAndFraming: string | null; cameraMotion: string | null; cinematographyJson: string | null; durationSeconds: number | null; narration: string | null; dialogue: string | null; visualContinuityNotes: string | null; productionStage: MovieProductionStage; clips: MovieClip[]; productionVersions: MovieProductionVersion[] };
...
export type MovieScreenplayElement = { id: string; ordinal: number; elementType: string; content: string; characterName: string | null; parenthetical: string | null };
...
export type MovieStoryRevision = { id: string; revisionNumber: number; parentRevisionId: string | null; createdByUserId: string; premise: string; logline: string; synopsis: string; treatment: string; status: string; authorship: string; changeSummary: string | null; rejectionReason: string | null; createdAt: string; updatedAt: string; submittedAt: string | null; approvedAt: string | null; approvedByUserId: string | null; scenes: MovieScreenplayScene[] };
export type MovieStory = { id: string; movieProjectId: string; workspaceId: string; premise: string; logline: string; synopsis: string; treatment: string; approvalState: string; currentRevisionId: string | null; approvedRevisionId: string | null; createdAt: string; updatedAt: string; currentRevision: MovieStoryRevision | null; approvedRevision: MovieStoryRevision | null; revisions: { id: string; revisionNumber: number; status: string; authorship: string; changeSummary: string | null; createdAt: string; submittedAt: string | null; approvedAt: string | null }[] };
export type DirectorStoryFieldChange = { field: string; target: string | null; existingContent: string; proposedContent: string };
export type DirectorStoryReview = { action: string; baseRevisionId: string | null; changes: DirectorStoryFieldChange[]; findings: string[]; appliesToStory: boolean };
...
export type DirectorProposal = { id: string; movieProjectId: string; status: string; title: string; summary: string; rationale: string[]; plan: unknown[]; actions: DirectorAction[]; createdAt: string; approvedAt: string | null; storyReview: DirectorStoryReview | null };
export type DirectorProposalResponse = { proposal: DirectorProposal; context: unknown; storyContext: unknown | null };
...
export const api = {
  ...
  createMovieDirectorProposal: (projectId: string, input: { storyAction: DirectorStoryAction; goal?: string | null; targetSceneId?: string | null; targetElementId?: string | null; selectedPassage?: string | null }) => request<DirectorProposalResponse>(`/api/movie-director/projects/${projectId}/proposals`, { method: "POST", body: JSON.stringify(input) }, true),
  approveMovieDirectorProposal: (proposalId: string) => request<DirectorProposal>(`/api/movie-director/proposals/${proposalId}/approve`, { method: "POST" }, true),
  rejectMovieDirectorProposal: (proposalId: string) => request<DirectorProposal>(`/api/movie-director/proposals/${proposalId}/reject`, { method: "POST" }, true),
  executeMovieDirectorAction: (actionId: string) => request<{ action: DirectorAction; result: { id: string; status: string; safeMessage: string; resultJson: string | null; createdAt: string } }>(`/api/movie-director/actions/${actionId}/execute`, { method: "POST" }, true),
  ...
  addMovieShot: (sceneId: string, input: { description: string; cameraAndFraming?: string | null; cameraMotion?: string | null; durationSeconds?: number | null; narration?: string | null; dialogue?: string | null; visualContinuityNotes?: string | null; cinematography?: CinematographyIntentSelection | null }) => request<MovieShot>(`/api/movie-studio/scenes/${sceneId}/shots`, { method: "POST", body: JSON.stringify(input) }, true),
