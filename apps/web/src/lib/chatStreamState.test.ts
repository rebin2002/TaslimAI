...
describe("Chat stream state", () => {
  ...
  it("applies the production event callback path to the visible message state", () => {
    let state = createChatStreamState();
    const visibleAssistantContent: string[] = [];
    const apply = (streamEvent: ChatStreamEvent) => {
      state = applyChatStreamEvent(state, streamEvent, next => {
        visibleAssistantContent.push(next.messages.find(item => item.id === "assistant-1")?.content ?? "");
      });
    };

    apply(event("message.started", { userMessage: user, assistantMessage: assistant }));
    apply(event("message.delta", { messageId: "assistant-1", delta: "1" }));
    apply(event("message.delta", { messageId: "assistant-1", delta: "," }));
    apply(event("message.delta", { messageId: "assistant-1", delta: " 2" }));
    apply(event("message.completed", { userMessage: user, assistantMessage: message("assistant-1", "Assistant", "1, 2", "Completed") }));
