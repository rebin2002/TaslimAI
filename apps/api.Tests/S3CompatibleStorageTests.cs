...
public sealed class S3CompatibleStorageTests
{
    ...
    {
        var client = new FakeObjectClient { Failure = new InvalidOperationException("r2-secret-value") };
        var logger = new RecordingLogger();
        var service = CreateService(client, logger);

        var exception = await Assert.ThrowsAsync<FileStorageUnavailableException>(() => service.StoreAsync("workspace/file", new MemoryStream([1, 2, 3])));
    ...
    private sealed class StorageProviderFactory(IReadOnlyDictionary<string, string?> values) : WebApplicationFactory<Program>
    {
